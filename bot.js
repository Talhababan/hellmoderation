...
client.on("message", message => {
    ...
    return message.channel.send(`Héll`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == ".tag") 
    return message.channel.send(`0007`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "ekiptag") 
    return message.channel.send(`0007`)
});
...
client.on("message", message => {
    if(message.content.toLowerCase() == "miwa") 
    return message.channel.send(`miwanıza goyim :heart:`)
});
...
client.on("message", async msg => {
 ...
 const i = await db.fetch(`kufur_${msg.guild.id}`)
    if (i == "acik") {
        const kufur = ["oç", "amk", "ananı sikiyim", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "sik", "yarrak", "am", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq",  "ak", "amq",];
